"""
Tests for utils.doubles module

These tests use a small dataset of 10 rows to verify the double setup strategy analysis functionality.
Run with: poetry run python tests/test_doubles.py
"""
...
def test_get_double_setup_strategies():
...
def test_strategy_names():
    ...
    # Check for EMA strategies
    assert 'EMA Aligned + SL < 10' in names
    assert 'EMA Aligned + No News' in names
    assert 'EMA Aligned + 50% Pullback' in names

    # Check for 30M strategies
    assert '30M Trend + SL < 10' in names
    assert '30M Trend + No News' in names
    assert '30M Trend + 50% Pullback' in names

    # Check for BOS strategies
    assert 'BOS + SL < 10' in names
    assert 'BOS + No News' in names
    assert 'BOS + 50% Pullback' in names
...
def test_calculate_strategy_statistics_basic():
    ...
    # Check columns exist if there are results
    if len(result) > 0:
...
def test_ema_aligned_sl_filter():
    """Test EMA Aligned + SL < 10 filter strategy."""
    sample_data = get_sample_data()
    strategies = get_double_setup_strategies()

    # Find the strategy
    strategy = [func for name, func in strategies if name == 'EMA Aligned + SL < 10'][0]
    filtered = strategy(sample_data)

    # Should only have rows where EMA == Direction AND SL < 10
...
def test_30m_trend_no_news_filter():
    """Test 30M Trend + No News filter."""
    sample_data = get_sample_data()
    strategies = get_double_setup_strategies()

    # Find the strategy
    strategy = [func for name, func in strategies if name == '30M Trend + No News'][0]
    filtered = strategy(sample_data)

    # Should only have rows where trend aligns AND no news
    assert len(filtered) >= 0
    for _, row in filtered.iterrows():
        ...
        assert pd.isna(row['News Event'])


def test_bos_pullback_filter():
    """Test BOS + 50% Pullback filter."""
    sample_data = get_sample_data()
    strategies = get_double_setup_strategies()

    # Find the strategy
    strategy = [func for name, func in strategies if name == 'BOS + 50% Pullback'][0]
    filtered = strategy(sample_data)

    # Should only have rows where BOS/CH == 'BOS' AND pullback >= 50% of SL
    assert len(filtered) >= 0
    for _, row in filtered.iterrows():
        assert row['BOS/CH'] == 'BOS'
        assert row['Pullback'] >= row['SL'] * 0.5
        assert row['Pullback'] >= 2
...
def test_ema_with_news_filter():
    """Test EMA Aligned + With News filter."""
    sample_data = get_sample_data()
    strategies = get_double_setup_strategies()

    # Find the strategy
    strategy = [func for name, func in strategies if name == 'EMA Aligned + With News'][0]
    filtered = strategy(sample_data)

    # Should only have rows where EMA == Direction AND news event exists
    assert len(filtered) >= 0
    for _, row in filtered.iterrows():
        assert row['EMA'] == row['Direction']
        assert not pd.isna(row['News Event'])
...
def run_all_tests():
    ...
    tests = [
        test_get_double_setup_strategies,
        test_strategy_names,
        ...
        test_ema_aligned_sl_filter,
        test_30m_trend_no_news_filter,
        test_bos_pullback_filter,
        test_sorting_by_edge,
        test_ema_with_news_filter,
        test_sl_range_filter,
    ]
