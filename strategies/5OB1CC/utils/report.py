"""
Static HTML report for the 5OB1CC strategy.

The same page as the 15LS1CC lab's 15C.html, rendered from this strategy's
data.csv. Every analysis table comes from utils.order_block, so the lab can be
read in a browser without a Jupyter kernel. Pair it with a file watcher to get
a live-updating page:
...
column the reader sorted.

The two pages share a build directory, so the sidecar files are named after
this page rather than reusing the 15C names.
"""
...
PAGE_TITLE = "5OB"
PAGE_SUBTITLE = ("5 minute order block with a 1 minute confirmation candle "
                 "&middot; EURUSD &middot; London session")
SOURCE_CSV = "strategies/5OB1CC/data.csv"

# Named after this page: the 15C report writes its own pair into the same
# build directory, and a shared name would have each reload the other's id.
BUILD_ID_FILENAME = "build-id-5ob.txt"
...
BUILD_ID_JS_FILENAME = "build-id-5ob.js"

# Milliseconds between change checks. The page only reloads on a real change.
POLL_INTERVAL_MS = 1000

SCROLL_STORAGE_KEY = "5ob1cc-scroll"
SORT_STORAGE_KEY = "5ob1cc-sorts"


def _weekday_section(df: pd.DataFrame) -> str:
    return create_html_table(
        calculate_weekday_statistics(df), first_col_width="40%")


def _hour_section(df: pd.DataFrame) -> str:
    return create_html_table(
        calculate_hour_statistics(df), sort_id="hour-table",
        first_col_width="40%")


def _ema_alignment_section(df: pd.DataFrame) -> str:
    return create_html_table(
        calculate_ema_alignment_statistics(df), first_col_width="40%")


def _structure_section(df: pd.DataFrame) -> str:
    return create_html_table(
        calculate_structure_statistics(df), first_col_width="40%")


def _htf_leg_section(df: pd.DataFrame) -> str:
    return create_html_table(
        calculate_htf_leg_statistics(df), first_col_width="40%")


def _sl_section(df: pd.DataFrame) -> str:
    return create_sortable_table(
        calculate_sl_statistics(df), "sl-range-table", sortable=False,
        first_col_width="40%")
...
# (anchor, nav label, heading, note, builder) - the 15C page's section order,
# with this export's own entry-time filters in the grouping block.
SECTIONS: List[Tuple[str, str, str, str, Callable[[pd.DataFrame], str]]] = [
    ...
    (
        ...
        "Hour",
        "Hour Analysis",
        "Trades split by entry hour in Lithuanian time, the same two readings "
        "as the weekday table. The hour is known before the trade is taken, so "
        "it is a tradeable filter. Default is every trade, and the Signal and "
        "Strategy headers are click-to-sort.",
        _hour_section,
    ),
    (
        "ema-alignment",
        ...
        "Trades split by whether they ran with the EMA signal or against it - "
        "this export's counterpart of the 15C page's 4H bias. Same two "
        "readings: <b>Signal</b> = TP &gt; 0 (the idea was right, stop "
        "ignored), <b>Strategy</b> = Pullback &lt; SL AND TP &gt;= SL (what "
        "trading it at 1:1 would have returned).",
        _ema_alignment_section,
    ),
    (
        "structure",
        "Structure",
        "Structure Analysis",
        "Break of structure against change of character. The structure type "
        "that produced the order block is known at entry, so it is a tradeable "
        "filter. Same Signal and Strategy pair as the weekday table.",
        _structure_section,
    ),
    (
        "htf-leg",
        "30M Leg",
        "30M Leg Analysis",
        "Where price sat relative to the 30-minute leg when the signal came. "
        "Same Signal and Strategy pair as the weekday table.",
        _htf_leg_section,
    ),
    (
        "sl-range",
        "SL Range",
        "SL Range Analysis",
        "Win rate per safe-stop band, under the same two readings as the "
        "weekday table: <b>Signal</b> = TP &gt; 0 (the idea was right, stop "
        "ignored), <b>Strategy</b> = Pullback &lt; SL AND TP &gt;= SL (what "
        "trading the band at 1:1 would have returned).",
        _sl_section,
    ),
    (
        "sl-fixed",
        ...
        "recorded stops as a baseline. <b>Signal</b> (TP &gt; 0) does not "
    ...
    (
        ...
        "R Distribution",
        "R Distribution",
        "R is derived as TP / SL - this export has no R column. Cumulative: a "
        "trade reaching 3R also passed through 1R and 2R.",
        create_r_histogram_combined,
    ),
    (
        ...
        "R Distribution (Exact)",
        "R Distribution (Exact)",
        "Each trade counted once, in the bucket of the highest whole R it "
        "reached.",
        create_r_histogram_exact,
    ),
]
...
def _reload_script(build_id: str) -> str:
    ...
    to loading the build-id script through a <script> tag, which is the one way
    a file:// page can read a sibling file. If neither works the page simply
    stops watching rather than reloading on a timer.
...
(function () {{
    ...
    var SCROLL_KEY = "{SCROLL_STORAGE_KEY}";
    var SORT_KEY = "{SORT_STORAGE_KEY}";
    ...
    function probeOverFile() {{
        ...
        s.onerror = function () {{
            ...
            setMode("auto-reload unavailable - refresh manually, "
                    + "or serve the folder over http:// (see labs/render_5OB.py)");
        }};
...
def build_report(df: pd.DataFrame, generated_at: str, build_id: str,
    ...
    """
    Build the full HTML document for the 5OB1CC lab.
...
<header style="margin-bottom: 24px;">
    <h1 style="color: {TEXT}; font-size: 22px; margin: 0 0 6px;">{PAGE_TITLE}</h1>
...
<footer style="color: {TEXT_MUTED}; font-size: 11px; border-top: 1px solid {BORDER};
    ...
    Generated by labs/render_5OB.py from {SOURCE_CSV}
</footer>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{PAGE_TITLE} Lab - build failed</title>
<style>
    body {{
        background-color: {BG_PAGE};
        color: {TEXT};
