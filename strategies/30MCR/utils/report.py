"""
Static HTML report for the 30MCR strategy.

The same page as the 15LS1CC lab's 15C.html, rendered from this strategy's
trades.csv. Every analysis table comes from utils.continuation_reversal, so the
lab can be read in a browser without a Jupyter kernel. Pair it with a file
watcher to get a live-updating page:
...
All three pages share a build directory, so the sidecar files are named after
this page rather than reusing the 15C or 5OB names.
"""
...
from utils.continuation_reversal import (
    RRR_RATIOS,
    THREE_SETUPS_RRR,
...
PAGE_TITLE = "30MCR"
PAGE_SUBTITLE = ("30 minute continuation and reversal setups "
                 "&middot; EURUSD &middot; London session")
SOURCE_CSV = "strategies/30MCR/trades.csv"

# Named after this page: the 15C and 5OB reports write their own pairs into
# the same build directory, and a shared name would have each page reload on
# another page's id.
BUILD_ID_FILENAME = "build-id-30mcr.txt"
...
BUILD_ID_JS_FILENAME = "build-id-30mcr.js"

# Milliseconds between change checks. The page only reloads on a real change.
POLL_INTERVAL_MS = 1000

SCROLL_STORAGE_KEY = "30mcr-scroll"
SORT_STORAGE_KEY = "30mcr-sorts"
...
def _setup_section(df: pd.DataFrame) -> str:
    return create_html_table(
...
SECTIONS: List[Tuple[str, str, str, str, Callable[[pd.DataFrame], str]]] = [
    ...
    (
        ...
        "R Distribution",
        "R Distribution",
        "R is derived as TP / SL - this export has no R column of its own. "
        "Cumulative: a "
    ...
    function probeOverFile() {{
        ...
        s.onerror = function () {{
            ...
            setMode("auto-reload unavailable - refresh manually, "
                    + "or serve the folder over http:// (see labs/render_30MCR.py)");
        }};
...
def build_report(df: pd.DataFrame, generated_at: str, build_id: str,
    ...
    """
    Build the full HTML document for the 30MCR lab.
...
<footer style="color: {TEXT_MUTED}; font-size: 11px; border-top: 1px solid {BORDER};
    ...
    Generated by labs/render_30MCR.py from {SOURCE_CSV}
</footer>
